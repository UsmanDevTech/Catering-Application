...
public class AccountsController : Controller
{
    ...
    public async Task<IActionResult> Login([FromQuery] string? ReturnUrl, LoginVM loginVM)
    {
        ...
        //checks user's roles
        if (user.LoginRole != Domain.Enums.UserType.Chef)
        {
            TempData["Error"] = "The user is not Authorized to access Chef panel";
            return View();
        }

        var organizations = _context.Organizations.Where(x => x.ChefId == user.Id).ToList();
        if(organizations.Count == 0)
        {
            TempData["Error"] = "You have not been Assigned any Organization Yet";
            return View();
        }
        ...
        {
            TempData["Error"] = "You have been Blocked by Admin";
            return View();
        }
    ...
    [Authorize]
    [HttpPost]
    public async Task<IActionResult> UpdateProfileAsync([FromQuery] string fullName, string phone, CancellationToken cancellationToken)
    {
        try
        {
            //gets user from DB
    ...
    internal async Task<string> UploadFile(IFormFile file)
    {
        ...
        string path_1 = Path.Combine(@"wwwroot", "UserImages");
        ...
        return BaseUrl() + "/UserImages/" + uniqueFileName;
    }
